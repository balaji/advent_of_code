...
find_houses([], _, _, S) ->
    io:format("~p~n", [sets:to_list(S)]),
    S;
find_houses([H | T], X, Y, S) ->
    case H of
        $^ ->
            find_houses(T, X, Y + 1, sets:add_element({X, Y + 1}, sets:add_element({X, Y}, S)));
        $> ->
            find_houses(T, X + 1, Y, sets:add_element({X + 1, Y}, sets:add_element({X, Y}, S)));
        $< ->
            find_houses(T, X - 1, Y, sets:add_element({X - 1, Y}, sets:add_element({X, Y}, S)));
        $v ->
            find_houses(T, X, Y - 1, sets:add_element({X, Y - 1}, sets:add_element({X, Y}, S)))
