...
execute([H | T], Array) ->
  case H of
    {toggle, A, B} -> execute(T, entertain(Array, A, B, fun(Index, Arr) -> array:get(Index, Arr) + 2 end));
    {off, A, B} -> execute(T, entertain(Array, A, B, fun(Index, Arr) -> max(0, array:get(Index, Arr) - 1) end));
    {on, A, B} -> execute(T, entertain(Array, A, B, fun(Index, Arr) -> array:get(Index, Arr) + 1 end))
  end.

entertain(Array, [X1, Y1], [X2, Y2], Fun) ->
  lists:foldl(fun(I, ArAcc) ->
    Row = array:get(I, ArAcc),
    Y = lists:foldr(fun(J, Acc) ->
      array:set(J, Fun(J, Acc), Acc)
                    end, Row, lists:seq(X1, X2)),
    array:set(I, Y, ArAcc)
              end, Array, lists:seq(Y1, Y2)).

check_lights_on(_, Index, Sum) when Index == 1000 -> Sum;
