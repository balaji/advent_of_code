...
main([FileName | _]) ->
    ...
    io:format("~p, ~p~n",
              [lists:sum([score(part1(I, [], {false, false})) || I <- L]),
               lists:sum([score({rule2(I), rule1(I)}) || I <- L])]).
...
make_vowel(A, Vowels) ->
    IsVowel = sets:is_element(A, sets:from_list([$a, $e, $i, $o, $u])),
    if IsVowel == true ->
           [A | Vowels];
       true ->
           Vowels
       ...
       true ->
           ...
           case length(Repeats) of
               0 ->
                   false;
               1 ->
                   length(string:split(L, lists:nth(1, Repeats), all)) > 2;
               _ ->
                   true
           end
    end.
...
to_list(T) ->
    lists:filter(fun(I) -> length(I) > 1 end,
                 lists:map(fun(I) -> string:slice(T, I, 2) end, lists:seq(0, length(T), 2))).

rule2([A, _, A | _]) ->
    true;
rule2([_ | T]) ->
    rule2(T);
rule2(L) when length(L) < 3 ->
    false.
