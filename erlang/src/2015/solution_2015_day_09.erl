...
distance(K, M) ->
    KeysList =
        lists:map(fun(L) ->
                  ...
                  K),
    lists:map(fun(Keys) -> lists:foldl(fun(Key, Acc) -> maps:get(Key, M) + Acc end, 0, Keys)
              end,
              KeysList).
...
solution(Inp) ->
    {M, L} = parse(Inp),
    Distances =
        distance(utils:permutations(
                     lists:uniq(L)),
                 M),
    [{part1, lists:min(Distances)}, {part2, lists:max(Distances)}].

run() ->
    solution(["London to Dublin = 464",
