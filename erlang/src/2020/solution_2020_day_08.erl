...
main([FileName | _]) ->
    L = array:map(
        ...
        array:from_list(utils:lines(FileName))
    ),
        ...
        _ ->
            case sets:is_element(Pointer, VisitedSet) of
                ...
                _ ->
                    ...
                    case Inst of
                        "nop" ->
                            execute(Code, Pointer + 1, Acc, NewVisit);
                        "jmp" ->
                            execute(Code, (Pointer + Argument) rem array:size(Code), Acc, NewVisit);
                        "acc" ->
                            execute(Code, Pointer + 1, Acc + Argument, NewVisit)
                    end
            end
    end.
