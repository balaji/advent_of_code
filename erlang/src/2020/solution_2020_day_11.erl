            ...
            case utils:array_get(A, I, J) of
                ...
                Y ->
                    case {Y, occupied(Fn(A, I, J, Li, Lj), 0)} of
                        {$#, X} when X >= Limit ->
                            game(
                                A, utils:array_set(Modified, I, J, $L), I, J + 1, {Li, Lj}, PartFn
                            );
                        {$L, X} when X == 0 ->
                            game(
                                A, utils:array_set(Modified, I, J, $#), I, J + 1, {Li, Lj}, PartFn
                            );
                        _ ->
                            game(A, Modified, I, J + 1, {Li, Lj}, PartFn)
                    end
...
part1(Arr, Pi, Pj, Li, Lj) ->
    lists:map(
        fun({A, B}) -> utils:array_get(Arr, A, B) end,
        lists:filter(
            fun({A, B}) -> (A < Li) and (B < Lj) and (A >= 0) and (B >= 0) end,
            [
                {Pi, Pj + 1},
                {Pi, Pj - 1},
                {Pi + 1, Pj},
