...
main([FileName | _]) ->
  ...
  calculate(lists:append(IntCode, array:to_list(array:new([{size, 50000}, {default, 0}]))), 0, 1, 0, 0).

calculate(IntCode, RelativeBase, Start, Step, Index) ->
  [Head | T] = lists:sublist(IntCode, Start, 4),
  [A, B, C, D, E] = [list_to_integer(integer_to_list(N - 48)) || N <- io_lib:format("~5..0B", [Head])],
  Code = D * 10 + E,
  case Code of
    ...
    X when X == 1; X == 2; X == 7; X == 8 ->
      ...
      calculate(replace_nth_value(IntCode, literal_find(A, RelativeBase, Pos3), Sum), RelativeBase, Start + 4, Step, Index);
    X when X == 3; X == 4; X == 9 ->
      [Param | _] = T,
