...
main([FileName | _]) ->
    ...
    Amplified = [
        calculate(Amplifiers, [A, B, C, D, E])
     || A <- PhaseCodes,
        B <- PhaseCodes,
        C <- PhaseCodes,
        D <- PhaseCodes,
...
calculate(Amplifiers, PhaseCodes) ->
    ...
    calculate(
        maps:put(lists:nth(1, PhaseCodes), {IntCode, Start, Input ++ [0]}, Amplifiers),
        PhaseCodes,
        0
    ).
...
calculate(Amplifiers, PhaseCodes, Output) ->
    ...
    case Code of
        ...
        X when X == 1; X == 2; X == 7; X == 8 ->
            ...
            calculate(
                maps:put(lists:nth(1, PhaseCodes), {NewIntCode, Start + 4, Input}, Amplifiers),
                PhaseCodes,
                Output
            );
        X when X == 3; X == 4 ->
            ...
            if
                X == 3 ->
                    ...
                    calculate(
                        maps:put(
                            lists:nth(1, PhaseCodes),
                            {NewIntCode, Start + 2, OtherInput},
                            Amplifiers
                        ),
                        PhaseCodes,
                        Output
                ...
                X == 4 ->
                    ...
                    calculate(
                        maps:put(lists:nth(1, PhaseCodes), {IntCode, Start + 2, Input}, Amplifiers),
                        [Second] ++ Tail ++ [First],
                        Num1
                    )
            end;
        X when X == 5; X == 6 ->
            ...
            if
                X == 5, Num1 /= 0 ->
                    calculate(
                        maps:put(lists:nth(1, PhaseCodes), {IntCode, Num2 + 1, Input}, Amplifiers),
                        PhaseCodes,
                        Output
                    );
                X == 6, Num1 == 0 ->
                ...
                true ->
                    calculate(
                        maps:put(lists:nth(1, PhaseCodes), {IntCode, Start + 3, Input}, Amplifiers),
                        PhaseCodes,
                        Output
                    )
            end;
