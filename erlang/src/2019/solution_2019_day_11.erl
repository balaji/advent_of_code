...
calculate(IntCode, RelativeBase, Pointer, InputOutput, Panel) ->
    ...
    [{A, []}, {B, []}, {C, []}, {D, []}, {E, []}] = [
        string:to_integer(integer_to_list(N - 48))
        ...
        X when X == 3; X == 4; X == 9 ->
            ...
            if
                X == 3 ->
                    ...
                    calculate(
                        replace_nth_value(IntCode, literal_find(C, RelativeBase, Param), Input),
                        RelativeBase,
                        Pointer + 2,
                        [],
                        NewPanel
                ...
                X == 4 ->
                    ...
                    calculate(
                        IntCode, RelativeBase, Pointer + 2, InputOutput ++ [Num1], PaintedPanel
                    );
                X == 9 ->
                    calculate(IntCode, RelativeBase + Num1, Pointer + 2, InputOutput, Panel)
            end;
        X when X == 5; X == 6 ->
            [Pos1, Pos2 | _] = T,
            Num1 = interpreted_find(C, RelativeBase, Pos1, IntCode),
...
make_panel(Panel, {D, C, {X, Y}}) ->
    Search = [
        {Direction, Color, {X1, Y1}}
     || {Direction, Color, {X1, Y1}} <- Panel, X == X1, Y == Y1
    ],
    if
        length(Search) > 1 ->
            io:format("unexpected fail~n");
        length(Search) == 1 ->
            [{_, Cx, _} | _] = Search,
            [{D, Cx, {X, Y}}] ++ Panel;
        true ->
...
change_direction(Number, Direction, {X, Y}) when Number == 0, Direction == north ->
    [west, {X - 1, Y}];
change_direction(Number, Direction, {X, Y}) when Number == 0, Direction == east ->
    [north, {X, Y + 1}];
change_direction(Number, Direction, {X, Y}) when Number == 0, Direction == west ->
    [south, {X, Y - 1}];
change_direction(Number, Direction, {X, Y}) when Number == 0, Direction == south ->
    [east, {X + 1, Y}];
change_direction(Number, Direction, {X, Y}) when Number == 1, Direction == north ->
    [east, {X + 1, Y}];
change_direction(Number, Direction, {X, Y}) when Number == 1, Direction == east ->
    [south, {X, Y - 1}];
change_direction(Number, Direction, {X, Y}) when Number == 1, Direction == west ->
    [north, {X, Y + 1}];
change_direction(Number, Direction, {X, Y}) when Number == 1, Direction == south ->
    [west, {X - 1, Y}].
