...
main([FileName | _]) ->
        ...
        fun({T, Str, [Num, Order]}) ->
            Inter = [
             ...
             || {A, _} <- lists:sort(
                    ...
                    maps:to_list(make_map(Str, maps:new()))
                )
            ],
            Sub = lists:sublist(Inter, 1, 5),
            if
                Sub == Order -> {defined, {T, Num}};
                true -> undefined
            end
        end,
        L
    ...
    io:format("~p~n", [
        lists:filter(
            fun({_, S}) -> string:rstr(S, "pole") /= 0 end,
            lists:map(
                fun(E) ->
                    dechiper(E)
                end,
...
shift([H | T], Num, Acc) ->
    case H of
        ...
        $- when Num div 2 /= 0 -> shift(T, Num, Acc ++ [$\s]);
        V ->
            Y = Num rem 26,
            if
                V + Y =< $z ->
...
make_map([H | T], Map) ->
    ...
    if
        IsKey == true -> make_map(T, maps:put(H, maps:get(H, Map) + 1, Map));
