...
main([FileName | _]) ->
  ...
  R = lists:sum(lists:flatten(lists:map(fun(X) ->
    lists:filter(fun(Item) -> not maps:is_key(Item, SeatMap) end, X) end, NearTickets))),
...
parse(Content) ->
  ...
  Rules = lists:map(
    fun(S) ->
      ...
      Range = [list_to_tuple(lists:map(
        fun(X) -> list_to_integer(X) end, string:split(R, "-"))) || R <- string:split(RangeText, " or ")],
      [Label, Range]
    end, string:tokens(L, "\n")),
...
part2(SeatMap, Rules, YourTicket, NearTickets) ->
  ValidSeats = lists:filter(fun(X) ->
    lists:foldl(fun(Item, Bool) -> Bool and maps:is_key(Item, SeatMap) end, true, X) end, NearTickets),
  ...
  TicketWithAllRules = lists:zip(lists:seq(1, length(Accumulator)), lists:foldl(
    fun(X, ListOfSets) ->
      lists:map(fun({Item, Set}) ->
          sets:intersection(sets:from_list(Item), Set)
        end, lists:zip(X, ListOfSets))
    end, Accumulator, WithApplicableRules)),
  
  TicketWithUniqueRule = find_unique(lists:sort(fun({_, Set1}, {_, Set2}) ->
    sets:size(Set1) < sets:size(Set2) end, TicketWithAllRules)),
  ...
  OnlyDepartures = lists:filter(
    ...
    end, TicketWithUniqueRule),
  lists:foldl(fun(I, Acc) -> I * Acc end, 1,
